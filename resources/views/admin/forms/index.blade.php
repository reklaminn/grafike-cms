    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        @forelse($forms as $form)
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow">
                <div class="flex items-start justify-between">
                    <div>
                        <h3 class="text-base font-semibold text-gray-800">{{ $form->name }}</h3>
                        <p class="text-xs text-gray-400 mt-1">{{ $form->description ?? 'Açıklama yok' }}</p>
                    </div>
                    @if($form->unread_count > 0)
                        <span class="px-2 py-1 bg-red-500 text-white text-xs font-bold rounded-full">{{ $form->unread_count }}</span>
                    @endif
                </div>
        ...
        @empty
            <div class="col-span-full text-center py-12 text-gray-400">
                <i class="fas fa-wpforms text-4xl mb-3"></i>
